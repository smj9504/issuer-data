...
def content_lines(pages: list[dict], *, band: float = 0.12) -> list[tuple[int, str]]:
    ...
    for page in pages:
        # Through reading_order, so the one definition of "content" is also
        # the one reading order: a two-column page reaches coverage in the
        # same sequence reflow emits it, not pdfplumber's interleaved y-sort.
