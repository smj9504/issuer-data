...
- **전체 시장 스캔**: 구현 완료. DART는 회사별 조회가 기본이라 종목 수 × API 호출이 되므로
  ...
  (8개월, 대형주 8종목 표본) → 전 종목 1년치도 하루 이틀 예산이면 끝난다. 아래 참고.
...
이를 코스피+코스닥 ≈ 2,600종목으로 확대하면:

- 8개월치: ≈ 15,000회 → **하루 예산 안에 들어온다**
- 1년치: ≈ 22,000회 → **이틀**

표본이 대형주라 공시가 median보다 많으므로 이 값은 **상한**으로 보면 된다.
HANDOFF에서 우려한 "최소 수만 회"보다 실제로는 훨씬 싸다. 이유는 `fetch_stake_changes`가
