...
변동이 통째로 소실. PK에 `stock_kind`를 넣고 재수집해 60→64행 복구를 확인했다.

이 수정은 SQLite 시절 `connect()`의 자동 재생성에 얹혀 있었지만, 그 장치는 PostgreSQL로
옮기며 **삭제됐다** — 공유 서버에서는 접속하는 프로세스마다 DROP/CREATE VIEW를 치게
되기 때문이다. 지금 `connect()`는 `schema_meta`의 버전을 읽어 **경고만** 하므로,
**옛 스키마의 DB는 `init-db`를 직접 돌려야 새 PK가 적용된다.** 돌리지 않으면 옛 PK가
그대로 남아 행이 계속 소실된다 (HANDOFF §5).
