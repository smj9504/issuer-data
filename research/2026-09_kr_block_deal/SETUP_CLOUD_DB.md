...
2. **Region: Northeast Asia (Seoul)** — 수집을 돌리는 머신에서 가까운 쪽. 종목마다
   DB 왕복이 있으니 왕복 **시간**은 종목 수만큼 곱해지지만(3,988회), 어차피 전체
   소요를 정하는 건 네트워크가 아니라 **OpenDART 일일 한도**다. 리전을 잘못 골라도
   이틀은 이틀이고, 그냥 굳이 먼 데 둘 이유가 없다는 정도의 선택이다.
   **데이터가 쌓이는 것과는 무관하다** — 접수번호 단위로 중복 제거되므로 몇 번을
   돌리든 같은 행은 하나다(§2.1, `upsert_coverage`).
